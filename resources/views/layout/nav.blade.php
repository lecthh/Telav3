<nav class="flex px-4 md:px-[100px] py-3.5 justify-between items-center bg-white text-[18px] font-inter text-cNot-black border border-b border-[#424242]/20 sticky top-0 z-50">
    <div class="flex gap-x-4 md:gap-x-6 items-center">
        <a wire:navigate href="{{ route('home') }}" class="flex-shrink-0">@include('svgs.logo')</a>
        <a wire:navigate href="{{ route('home') }}" class="hover:text-cPrimary transition-colors">Home</a>
        <a wire:navigate href="{{ route('customer.place-order.select-apparel') }}" class="hover:text-cPrimary transition-colors hidden md:block">Start Your Custom Order</a>
    </div>
    <div class="flex gap-x-3 md:gap-x-6 items-center" x-data="{ isOpen: false, userMenuOpen: false }">
        <div class="flex gap-x-3 md:gap-x-4">
            ...
            <button class="relative hover:text-cPrimary transition-colors" @click.prevent="isOpen = !isOpen">
                @include('svgs.bell')
                @php
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open" class="flex items-center gap-x-2 hover:text-cPrimary transition-colors">
