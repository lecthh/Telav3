<div class="flex flex-col border-r border-gray-200 h-full min-h-screen w-[280px] bg-white shadow-sm">
    ...
    <div class="py-4 px-2 flex-grow overflow-y-auto">
        <nav class="flex flex-col space-y-1">
            ...
            @php
                // Get the production company user ID
                $productionCompanyUserId = null;
                if (is_object($productionCompany) && isset($productionCompany->user_id)) {
                    $productionCompanyUserId = $productionCompany->user_id;
                } elseif (is_array($productionCompany) && isset($productionCompany['App\\Models\\ProductionCompany'])) {
                    $productionCompanyUserId = $productionCompany['App\\Models\\ProductionCompany']['user_id'] ?? null;
                    ...
                    $productionCompanyUserId = $pcData->user_id ?? ($pcData['user_id'] ?? null);
                }
                
                // Get unread notification count
                $unreadCount = 0;
                if ($productionCompanyUserId) {
                    $unreadCount = \App\Models\Notification::where('user_id', $productionCompanyUserId)
                        ->where('is_read', false)
                        ->count();
                }
            @endphp
            
            <a href="{{ route('partner.printer.profile.notifications') }}" class="flex items-center gap-x-3 px-3 py-2.5 rounded-md {{ request()->routeIs('partner.printer.profile.notifications') ? 'bg-cAccent/20 text-cAccent font-medium' : 'text-gray-700 hover:bg-cAccent/10' }} transition-colors">
                <div class="relative">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <a href="{{ route('partner.printer.profile.reviews') }}" class="flex items-center gap-x-3 px-3 py-2.5 rounded-md {{ request()->is('*/profile/reviews') ? 'bg-cAccent/20 text-cAccent font-medium' : 'text-gray-700 hover:bg-cAccent/10' }} transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <a href="{{ route('partner.printer.profile.transactions') }}" class="flex items-center gap-x-3 px-3 py-2.5 rounded-md {{ request()->routeIs('partner.printer.profile.transactions') ? 'bg-cAccent/20 text-cAccent font-medium' : 'text-gray-700 hover:bg-cAccent/10' }} transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <a href="{{ route('partner.printer.designers') }}" class="flex items-center gap-x-3 px-3 py-2.5 rounded-md text-gray-700 hover:bg-cAccent/10 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="pt-4 mt-4 border-t border-gray-200">
                <a href="{{ route('partner.printer.profile.basics') }}" class="flex items-center gap-x-3 px-3 py-2.5 rounded-md {{ (request()->routeIs('partner.printer.profile.basics') || request()->routeIs('partner.printer.profile.pricing')) ? 'bg-cAccent/20 text-cAccent font-medium' : 'text-gray-700 hover:bg-cAccent/10' }} transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
