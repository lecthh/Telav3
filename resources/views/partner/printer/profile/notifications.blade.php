                    ...
                    <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <form action="{{ route('partner.printer.profile.notifications.mark-all-read') }}" method="POST" class="ml-auto">
                                @csrf
                                <button type="submit" class="text-sm text-cPrimary hover:underline">Mark all as read</button>
                            </form>
                            @endif
                        </div>
                        ...
                        <div class="divide-y divide-gray-100">
                            ...
                            <div class="p-4 hover:bg-gray-50 flex {{ $notification->is_read ? 'opacity-75' : 'bg-cPrimary/5' }}">
                                <div class="flex-shrink-0 mr-4">
                                    @if(Str::contains(strtolower($notification->message), 'review'))
                                    <span class="inline-flex items-center justify-center h-10 w-10 rounded-full bg-cPrimary/10 text-cPrimary">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                        </svg>
                                    </span>
                                    @elseif(Str::contains(strtolower($notification->message), 'order'))
                                ...
                                <div class="flex-shrink-0 ml-4 self-center">
                                    <form action="{{ route('partner.printer.profile.notifications.mark-read', $notification->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="text-xs text-gray-500 hover:text-cPrimary">
                                            Mark as read
                                        </button>
                                    </form>
                                </div>
