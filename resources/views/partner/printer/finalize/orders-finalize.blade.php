                    ...
                    <table class="table-auto min-w-full overflow-hidden rounded-lg animate-fade-in">
                        <thead class="border drop-shadow-sm">
                            <tr class="bg-cPrimary font-bold text-base text-white">
                                ...
                                <th class="px-5 py-[14px] text-start">Filled</th>
                                <th class="px-5 py-[14px] text-start">Designer</th>
                            </tr>
                        </thead>
                        <tbody class="border drop-shadow-sm">
                            @if($finalizeOrders->isEmpty())
                            <tr>
                            ...
                            @else
                            @foreach($finalizeOrders as $order)
                            <tr class="odd:bg-gray-100 even:bg-white hover:bg-cAccent hover:bg-opacity-10 cursor-pointer" data-url="{{ route('partner.printer.finalize-x', ['order_id' => $order->order_id]) }}">
